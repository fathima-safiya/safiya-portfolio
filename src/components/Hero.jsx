...
const Hero = () => {
  ...
  return (
    <section
      id="home"
      ref={sectionRef}
      className="relative w-full bg-slate-900 overflow-hidden flex flex-col justify-between select-none pb-8 sm:pb-12"
    >
      <style>{`
        @keyframes marquee {
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden opacity-10">
          <div className="flex whitespace-nowrap animate-marquee">
            {[...developerRoles, ...developerRoles].map((role, idx) => (
              <span key={idx} className="text-[14vw] font-black text-emerald-500 mx-8 uppercase tracking-tighter">
                {role} &bull;
              </span>
            ))}
          </div>
      ...
      <header className="absolute inset-x-0 top-0 z-50 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-4 sm:py-6 flex items-center justify-between pointer-events-auto">
        <a href="#home" className="text-xl sm:text-2xl font-black text-emerald-500 tracking-tighter flex items-center gap-1.5 drop-shadow-[0_2px_15px_rgba(16,185,129,0.9)]">
        ...
        <div className="flex items-center gap-2.5 sm:gap-4">
          <a
            ...
            className="flex items-center gap-1.5 sm:gap-2.5 px-3.5 sm:px-5 md:px-6 py-2 sm:py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 text-emerald-300 font-bold text-xs sm:text-[13px] uppercase tracking-widest transition-all duration-300 shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.4)] hover:scale-105 active:scale-95 backdrop-blur-md group"
          >
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg bg-white/5 border border-white/10 text-white/80 hover:text-white hover:border-emerald-500/50 transition-colors focus:outline-none"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-x-0 top-[68px] z-40 bg-slate-900/95 backdrop-blur-3xl border-b border-white/10 p-6 flex flex-col gap-4 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="flex flex-col gap-3 text-sm font-mono uppercase tracking-widest text-white/80">
            <a 
              href="#home" 
              onClick={() => setMobileMenuOpen(false)}
              className="py-2.5 px-4 rounded-lg hover:bg-emerald-500/10 hover:text-emerald-300 transition-colors"
            >
              Home
            </a>
            <a 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-7 flex flex-col items-start space-y-4 sm:space-y-5 text-left">
            ...
            <div className="hero-anim-item flex flex-col items-start gap-2.5 sm:gap-3">
              <span className="px-3 sm:px-4 py-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-bold text-xs sm:text-sm md:text-base uppercase rounded tracking-widest shadow-[0_0_15px_rgba(16,185,129,0.15)] animate-pulse w-fit">OPEN TO WORK</span>
