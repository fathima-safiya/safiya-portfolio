...
const MinimalPreloader = ({ onComplete }) => {
  ...
  return (
    <div
      ref={preloaderRef}
      className="fixed inset-0 z-[9999] bg-slate-900 flex flex-col items-center justify-center select-none overflow-hidden"
    >
      <div className="flex flex-col items-center text-center px-4 sm:px-6 relative z-10">
        ...
        <div className="mb-5 sm:mb-6 flex justify-center items-center">
          <div className="relative w-12 h-12 sm:w-14 sm:h-14 flex justify-center items-center animate-bounce" style={{ animationDuration: '3s' }}>
            ...
            {/* Geometric Cube SVG */}
            <svg className="w-8 h-8 sm:w-10 sm:h-10 text-emerald-400 relative z-10 drop-shadow-[0_0_15px_rgba(16,185,129,0.8)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5">
        ...
        <div className="w-64 sm:w-80 md:w-96 flex flex-col items-center mt-2 sm:mt-4">
          ...
          <div className="w-full flex justify-end items-center mb-3 sm:mb-4">
            <span className="text-sm sm:text-base font-mono font-black text-emerald-400 tracking-widest drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]">
              {progress}%
            </span>
          </div>

          {/* Premium Progress Line */}
          <div className="w-full h-1.5 sm:h-2 bg-slate-800 border border-white/5 rounded-full overflow-hidden relative shadow-inner">
            <div 
              ref={progressRef}
              className="absolute top-0 left-0 h-full bg-gradient-to-r from-teal-500 to-teal-300 rounded-full transition-all duration-200 ease-out shadow-[0_0_20px_rgba(16,185,129,1)]"
              style={{ width: '0%' }}
            >
