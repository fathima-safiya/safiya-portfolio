...
const skillCategories = [
  { 
    ...
    desc: 'Crafting responsive and interactive user interfaces using modern web technologies.', 
    tag: 'UI / INTERACTION',
    skills: ['HTML', 'CSS', 'JavaScript', 'React', 'TypeScript', 'Tailwind CSS'] 
  },
  { 
    title: 'Programming & Backend', 
    desc: 'Building secure server-side applications and core logic.', 
    tag: 'ARCHITECTURE',
    skills: ['Java', 'PHP', 'C#', 'Node.js'] 
  },
  { 
    title: 'Databases', 
    desc: 'Designing schemas and managing data effectively.', 
    tag: 'DATA',
    skills: ['MySQL', 'Firebase Firestore', 'SQLite'] 
  },
  { 
    title: 'Tools & Ecosystem', 
    desc: 'Equipped with industry-grade instruments for version control and productivity.', 
    tag: 'PRODUCTIVITY',
    skills: ['Git', 'GitHub', 'VS Code', 'Android Studio'] 
  },
  { 
    title: 'Cloud (Currently Learning)', 
    desc: 'Expanding knowledge in deployment, scaling, and cloud infrastructure.', 
    tag: 'INFRASTRUCTURE',
    skills: ['AWS', 'Cloud Computing', 'Docker', 'CI/CD', 'Firebase'] 
  },
];
      ...
      <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none">
        {skillCategories.map((_, i) => (
          <h1 
            ...
            style={{ 
               WebkitTextStroke: `2px ${i % 2 === 0 ? 'rgba(20,184,166,0.3)' : 'rgba(255,255,255,0.15)'}`,
               opacity: 0 
            }}
          >
            SKILLS
        ...
        {skillCategories.map((category, i) => (
          ...
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              <span className="text-[10px] font-mono font-bold tracking-widest uppercase text-teal-400 bg-teal-500/10 px-3 py-1 rounded border border-teal-500/20">
                {category.tag}
              </span>
            ...
            <div className="space-y-4 relative z-10 my-auto">
              <h3 className="text-3xl md:text-4xl font-black text-white tracking-tight group-hover:text-teal-400 transition-colors duration-300">
                {category.title}
              </h3>
            ...
            <div className="flex flex-wrap gap-2 pt-4 border-t border-white/10 relative z-10">
              {category.skills.map((skill, sIdx) => (
                <span 
                  key={sIdx}
                  className="text-xs font-mono text-white/70 bg-white/5 border border-white/10 px-3 py-1 rounded group-hover:border-teal-500/30 transition-colors"
                >
                  {skill}
                </span>
              ))}
